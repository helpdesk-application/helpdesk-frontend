...
const TicketList = () => {
  ...
  useEffect(() => {
    const mockTickets = [
      { id: 'TICK-101', subject: 'Laptop not starting', status: 'Open', priority: 'High', category: 'Hardware', createdAt: '2023-10-01' },
      { id: 'TICK-102', subject: 'VPN Access Issue', status: 'In-Progress', priority: 'Medium', category: 'Network', createdAt: '2023-10-02' },
      { id: 'TICK-103', subject: 'Software Install', status: 'Resolved', priority: 'Low', category: 'Software', createdAt: '2023-10-03' },
      { id: 'TICK-104', subject: 'Printer Offline', status: 'Open', priority: 'Medium', category: 'Hardware', createdAt: '2023-10-04' },
    ];
          ...
          {['All', 'Open', 'In-Progress', 'Resolved'].map((s) => (
            <button
              ...
              className={`px-5 py-2 rounded-lg text-sm font-medium whitespace-nowrap transition-all ${
                filter === s ? 'bg-white shadow-md text-blue-600' : 'text-slate-500 hover:text-slate-700'
              }`}
            >
              {s}
            </button>
